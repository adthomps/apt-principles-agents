...
This assessment captures the repository before the agent harness migration. It is an implementation inventory, not a replacement for canonical APT doctrine in `apt-principles`.
...
| `templates/` | Starter manifest template for legacy `.agent-standards.json`. |
...
| `apt-principles-reviewer` | Reviewer | Reviews APT alignment and behavior preservation. | References doctrine; does not own doctrine. |
...
| Script | Purpose |
| --- | --- |
| `install-agent-standards.mjs` | Installs selected profiles and writes legacy `.agent-standards.json`. |
...
Gaps: model detection, model-routing validation, new `.agent-repo` manifests, scan reports, repair plans, backup-aware repair, and harness sync reports.
